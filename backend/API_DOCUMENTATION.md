...
| `/upload` | `POST multipart/form-data` | ต้อง | ไม่ใช้ secure JSON wrapper | รับไฟล์และ enqueue sanitize/upload job |
| `/upload/jobs/:jobId` | `GET` | ต้อง | ไม่ต้อง | อ่านสถานะ upload sanitize job |
...
1. `POST /upload` ตอบ `202` พร้อม `job.id`
2. client poll `GET /upload/jobs/:jobId` จน `status=succeeded`
3. เมื่อไฟล์พร้อมแล้วค่อยเรียก validation หรือ merge
4. `POST /documents/merge` ตอบ `202` พร้อม `job.id`

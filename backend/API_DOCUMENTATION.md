...
- [PFS_PROTOCOL_V2.md](/Users/pst./senior/backend/PFS_PROTOCOL_V2.md)
...
- runtime ของ app และ worker ถูกแยก `DOCUMENT_INTAKE_KMS_ACCESS_MODE` เป็น `encrypt` และ `decrypt` ตามลำดับเพื่อลด capability ของแต่ละ service
- document worker รันบน distroless/nonroot image และ deploy ด้วย memory/timeout/CPU/concurrency policy แยกจาก API หลัก
- worker สามารถต่อ AV scanning chain ก่อนขั้น sanitize ได้ผ่าน `DOCUMENT_AV_SCAN_MODE`, `DOCUMENT_AV_SCAN_URL`, และ `DOCUMENT_AV_SCAN_TIMEOUT_MS`
- ถ้าเปิด AV mode โดยยังไม่มี URL และเลือกใช้ deploy scaffold ของระบบ `deploy.sh` สามารถสร้าง private document AV scanner service ให้เองและ derive `DOCUMENT_AV_SCAN_URL`/audience กลับเข้า worker ได้
...
- secure transport รุ่นถัดไปที่มี PFS ถูกแยกเป็น design doc และ backend รองรับ handshake + protected route transport แล้วใน [PFS_PROTOCOL_V2.md](/Users/pst./senior/backend/PFS_PROTOCOL_V2.md)
...
| `/upload` | `POST multipart/form-data` | ต้อง | ไม่ใช้ secure JSON wrapper | รับไฟล์และ stage ไว้แบบปลอดภัยก่อนตรวจสอบ |
| `/upload/jobs/:jobId` | `GET` | ต้อง | ไม่ต้อง | อ่านสถานะ document preparation job สำหรับ upload/validation flow |
...
- protected JSON endpoints สามารถรับ envelope แบบ `v2` ได้แล้ว
- rollout ฝั่ง client/BFF ยังเป็นแบบ opt-in และ `v1` ยังทำงานคู่กันได้

## Async Document Jobs

flow ใหม่สำหรับงานหนัก:

1. `POST /upload` ตอบ `200 success` พร้อม staged file metadata
2. ตอนกด `POST /validation/check-completeness` ถ้าไฟล์ยังไม่พร้อม backend จะตอบ `202 queued` พร้อม batch `job`
3. client poll `GET /upload/jobs/:jobId` จน `status=succeeded` หรือ `status=partial_failed`
4. เมื่อ prepare job สำเร็จ backend จะตรวจ validation ต่อ
...
- backend จะ stage upload ทันที และ enqueue job ตอน validation/merge เฉพาะส่วนที่ต้องประมวลผลหนัก
...
- ถ้าเปิด AV scan แบบ `required` งานจะ fail ด้วย `422` เมื่อ scanner ระบุว่าไฟล์ไม่ clean และจะ fail ด้วย `503` ถ้า scanner ใช้งานไม่ได้
- ถ้าเปิด AV scan แบบ `log-only` worker จะ log ความผิดปกติแต่ยังเดินหน้าสู่ขั้น sanitize ต่อ

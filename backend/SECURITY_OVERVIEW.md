...
store ของ rate limit สามารถย้ายไปใช้ Upstash Redis ได้ผ่าน `UPSTASH_REDIS_REST_URL` และ `UPSTASH_REDIS_REST_TOKEN`
โดย design ปัจจุบันจะใช้ Redis สำหรับ hot-path state อย่าง rate limit และ nonce replay cache เมื่อ config ครบ และจะ fallback เป็น Firestore เฉพาะกรณีที่ยังไม่ได้เปิด Redis เท่านั้น
nonce replay cache จะยังคง fail closed ถ้า Redis ถูกเปิดใช้งานอยู่แต่มี error เพื่อไม่ให้ security ลดลง

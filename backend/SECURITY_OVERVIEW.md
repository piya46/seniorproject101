...
และ runtime ของ protected endpoints สามารถรับ envelope แบบ `v2` ได้แล้ว โดย derive request/response keys แยกกันใน middleware
...
response ที่ผ่าน authenticated middleware จะถูกบังคับ no-store/no-cache เพิ่มอีกชั้นหนึ่งผ่าน

- `Cache-Control: private, no-store, max-age=0, must-revalidate`
- `Pragma: no-cache`
- `Expires: 0`

เพื่อให้ browser/intermediary ไม่ cache ข้อมูลหลังบ้านที่ผูกกับ session นานเกินจำเป็น
...
และล่าสุด logout response จะส่ง `Clear-Site-Data: "cache", "cookies", "storage"` เพื่อช่วยล้าง browser-side state ที่ค้างอยู่ให้แรงขึ้นใน browser ที่รองรับ
...
เมื่อเปิด `PFS_V2_ENABLED=true`:

- client สามารถเริ่มด้วย `GET /api/v2/auth/handshake`
- protected endpoints สามารถรับ envelope แบบ `v2`
- backend จะ derive request/response keys แยกกันจาก ephemeral exchange แทนการ reuse symmetric key เดียวทั้งสองขา
...
- trusted BFF requests สามารถ harden เพิ่มได้ด้วย `TRUSTED_BFF_REQUIRE_IDENTITY_TOKEN=true` เพื่อบังคับ Google-signed service identity token เพิ่มจาก shared secret
...
runtime ปัจจุบันใช้ Helmet/CSP ฝั่ง backend และเพิ่ม browser hardening headers ที่สำคัญ เช่น
...
แนวทางนี้ตั้งใจให้ public `run.app` entrypoint แข็งขึ้นโดยไม่ต้องพึ่ง LB/IAP layer ก่อน
...
ล่าสุด document preparation ตอน validation และ document merge ถูกย้ายไป async job model แล้ว ทำให้ API หลักหยุดทำงานหนักใน request cycle และลดความเสี่ยง OOM/timeout จากงานเอกสารลงอีกชั้นหนึ่ง
raw intake object ที่ต้องค้างรอ worker จะถูกเข้ารหัสระดับแอปแบบ KMS envelope encryption ก่อนเก็บลง GCS และถูกลบทิ้งหลัง worker ทำงานจบหรือ fail เพื่อลด exposure window ของไฟล์ระหว่างทาง
worker runtime ถูก harden เพิ่มด้วย distroless image, nonroot user, application tree แบบ read-only และ Cloud Run resource policy ที่บังคับ memory/timeout/concurrency ต่ำสำหรับงานประมวลผลเอกสาร
ถ้า operator เปิด `DOCUMENT_AV_SCAN_MODE=log-only` หรือ `required` worker จะส่งไฟล์ต้นฉบับไปยัง AV scanning service ก่อน sanitize เพื่อช่วยลดความเสี่ยงจาก malformed document หรือ malware chain ที่อาจโจมตีไลบรารีประมวลผลไฟล์
deploy ปัจจุบันสามารถ scaffold private Cloud Run AV scanner service ให้เองได้ และ worker จะเรียกผ่าน Google-signed identity token; อย่างไรก็ดีถ้ายังไม่ได้ตั้ง `CLAMAV_HOST` scanner ที่ได้จะเป็น baseline service ที่ตรวจ EICAR fallback และทำหน้าที่เป็นจุดต่อสำหรับ AV engine จริงในภายหลัง
...
- rate limit และ nonce cache ปัจจุบันยังใช้ Firestore hot path อยู่
- จึงควรมองว่า availability/cost-exhaustion resilience ยังพึ่ง operational monitoring และ threshold tuning ควบคู่กัน
- ถ้าระบบโตขึ้นหรือโดน abuse บ่อย ควรพิจารณา edge protection หรือ cache store ที่เหมาะกับ hot counter workload มากกว่า Firestore
...
สำหรับการทำซ้ำในโปรเจกต์ ให้ใช้ [scripts/createSecurityLogMetrics.sh](./scripts/createSecurityLogMetrics.sh) เพื่อ scaffold log-based metrics ที่จะเอาไปผูกเป็น alerting policies ต่อใน Cloud Monitoring

ถ้าต้องการให้ขั้นตอน alerting ใกล้ one-click ขึ้นอีก ให้ใช้ [scripts/createSecurityAlertPolicies.sh](./scripts/createSecurityAlertPolicies.sh) ต่อจาก metrics script โดยกำหนด notification channels และ threshold ผ่าน env ได้

deploy ล่าสุดสามารถเรียกสองสคริปต์นี้ผ่าน `deploy.sh` ได้อัตโนมัติแล้ว โดยใช้ env:

- `ENABLE_SECURITY_LOG_METRICS=true`
- `ENABLE_SECURITY_ALERT_POLICIES=true`
- `SECURITY_ALERT_NOTIFICATION_CHANNELS=<resource names>`

หรือถ้าต้องการให้สคริปต์พยายามหา/reuse หรือสร้าง email notification channel ให้ก่อน:

- `AUTO_CREATE_SECURITY_NOTIFICATION_CHANNELS=true`
- `SECURITY_ALERT_NOTIFICATION_EMAILS=<comma-separated emails>`

ข้อควรทราบ:

- email notification channel บางโปรเจกต์อาจยังต้อง verify ปลายทางหรือใช้สิทธิ์ Monitoring ที่เพียงพอ
- ถ้า auto-create ไม่สามารถ resolve channel ได้จริง สคริปต์จะ fail ชัดเจนแทนที่จะผ่านเงียบ ๆ
- Cloud Monitoring CLI ที่ใช้กับ channels ใน environment นี้ยังต้องอาศัย `gcloud beta monitoring channels ...`

ถ้าต้องการกัน Layer 7 abuse ก่อนถึง Cloud Run ให้ดู [CLOUD_ARMOR_STANDARD.md](./CLOUD_ARMOR_STANDARD.md) เพิ่มเติม โดยเฉพาะเงื่อนไขเรื่อง external load balancer/serverless NEG
...
## DB Encryption Migration Notes

ถ้าจะหมุน DB encryption key ในอนาคต ให้ใช้แนวทางนี้:

1. เพิ่ม key version ใหม่เข้า runtime ก่อน เช่น `DB_ENCRYPTION_KEY_V2`
2. คง `DB_ENCRYPTION_KEY_VERSION` เดิมไว้ระหว่าง migration
3. ใช้ script migration แบบ dry-run เพื่อนับ backlog ของ legacy / old-version docs
4. ใช้ write mode เพื่อ re-encrypt fields ให้เป็น version ใหม่
5. ตรวจว่า legacy backlog เหลือศูนย์ก่อนค่อย flip default version

เหตุผลที่ต้องทำตามลำดับนี้:

- legacy payload เดิมไม่มี KID
- runtime จึงต้องอาศัย current DB key ในการถอดรหัส legacy docs
- ถ้า flip default เร็วเกินไป legacy docs อาจ decrypt ไม่ผ่านจนกระทบ file lookup และ merge flows

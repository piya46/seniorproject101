...
DOCUMENT_INTAKE_KEY_SECRET="${DOCUMENT_INTAKE_KEY_SECRET:-DOCUMENT_INTAKE_ENCRYPTION_KEY}"
...
deploy_document_job_worker_service() {
    ...
    gcloud run deploy "$DOCUMENT_JOB_WORKER_SERVICE_NAME" \
        ...
        --set-secrets "DOCUMENT_INTAKE_ENCRYPTION_KEY=${DOCUMENT_INTAKE_KEY_SECRET}:latest"
}
...
ensure_app_service_account_and_permissions() {
    ...
    grant_secret_accessor_on_secret "$DOCUMENT_INTAKE_KEY_SECRET" "$APP_SERVICE_ACCOUNT_EMAIL"
...
# 5.2.1 Document Intake Encryption Key (สุ่ม Hex 32 bytes)
RANDOM_DOCUMENT_INTAKE_KEY=$(openssl rand -hex 32)
create_secret_if_missing $DOCUMENT_INTAKE_KEY_SECRET "$RANDOM_DOCUMENT_INTAKE_KEY"

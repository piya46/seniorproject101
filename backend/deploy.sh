...
DOCUMENT_JOB_WORKER_SERVICE_ACCOUNT_NAME="${DOCUMENT_JOB_WORKER_SERVICE_ACCOUNT_NAME:-${APP_NAME}-docwrk-sa}"
...
PFS_V2_ENABLED="${PFS_V2_ENABLED:-false}"
PFS_V2_HANDSHAKE_TTL_MS="${PFS_V2_HANDSHAKE_TTL_MS:-300000}"
...
TRUST_PROXY_BROWSER_ORIGIN_HEADER="${TRUST_PROXY_BROWSER_ORIGIN_HEADER:-true}"
DEPLOY_NON_INTERACTIVE="${DEPLOY_NON_INTERACTIVE:-false}"
...
ENABLE_SECURITY_LOG_METRICS="${ENABLE_SECURITY_LOG_METRICS:-true}"
ENABLE_SECURITY_ALERT_POLICIES="${ENABLE_SECURITY_ALERT_POLICIES:-false}"
SECURITY_ALERT_POLICY_PREFIX="${SECURITY_ALERT_POLICY_PREFIX:-Sci Request}"
SECURITY_ALERT_NOTIFICATION_CHANNELS="${SECURITY_ALERT_NOTIFICATION_CHANNELS:-}"
AUTO_CREATE_SECURITY_NOTIFICATION_CHANNELS="${AUTO_CREATE_SECURITY_NOTIFICATION_CHANNELS:-false}"
SECURITY_ALERT_NOTIFICATION_EMAILS="${SECURITY_ALERT_NOTIFICATION_EMAILS:-}"
...
AI_CHAT_DAILY_TOKEN_LIMIT="${AI_CHAT_DAILY_TOKEN_LIMIT:-$AI_DAILY_TOKEN_LIMIT}"
AI_VALIDATION_DAILY_TOKEN_LIMIT="${AI_VALIDATION_DAILY_TOKEN_LIMIT:-$AI_DAILY_TOKEN_LIMIT}"
AI_USAGE_RETENTION_DAYS="${AI_USAGE_RETENTION_DAYS:-30}"
BACKEND_TMPDIR="${BACKEND_TMPDIR:-/tmp}"
DOCUMENT_JOB_WORKER_TMPDIR="${DOCUMENT_JOB_WORKER_TMPDIR:-/tmp}"
DOCUMENT_AV_SCANNER_TMPDIR="${DOCUMENT_AV_SCANNER_TMPDIR:-/tmp}"
CLEANUP_SERVICE_TMPDIR="${CLEANUP_SERVICE_TMPDIR:-/tmp}"
...
PFS_V2_ENABLED="$(trim_value "$PFS_V2_ENABLED")"
PFS_V2_HANDSHAKE_TTL_MS="$(trim_value "$PFS_V2_HANDSHAKE_TTL_MS")"
...
TRUSTED_BFF_REQUIRE_IDENTITY_TOKEN="$(trim_value "$TRUSTED_BFF_REQUIRE_IDENTITY_TOKEN")"
TRUSTED_BFF_IDENTITY_TOKEN_HEADER="$(trim_value "$TRUSTED_BFF_IDENTITY_TOKEN_HEADER")"
...
DEPLOY_NON_INTERACTIVE="$(trim_value "$DEPLOY_NON_INTERACTIVE")"
...
DOCUMENT_JOB_WORKER_SERVICE_ACCOUNT_NAME="$(trim_value "$DOCUMENT_JOB_WORKER_SERVICE_ACCOUNT_NAME")"
...
AI_CHAT_DAILY_TOKEN_LIMIT="$(trim_value "$AI_CHAT_DAILY_TOKEN_LIMIT")"
AI_VALIDATION_DAILY_TOKEN_LIMIT="$(trim_value "$AI_VALIDATION_DAILY_TOKEN_LIMIT")"
AI_USAGE_RETENTION_DAYS="$(trim_value "$AI_USAGE_RETENTION_DAYS")"
ENABLE_SECURITY_LOG_METRICS="$(trim_value "$ENABLE_SECURITY_LOG_METRICS")"
ENABLE_SECURITY_ALERT_POLICIES="$(trim_value "$ENABLE_SECURITY_ALERT_POLICIES")"
SECURITY_ALERT_POLICY_PREFIX="$(trim_value "$SECURITY_ALERT_POLICY_PREFIX")"
SECURITY_ALERT_NOTIFICATION_CHANNELS="$(trim_value "$SECURITY_ALERT_NOTIFICATION_CHANNELS")"
AUTO_CREATE_SECURITY_NOTIFICATION_CHANNELS="$(trim_value "$AUTO_CREATE_SECURITY_NOTIFICATION_CHANNELS")"
SECURITY_ALERT_NOTIFICATION_EMAILS="$(trim_value "$SECURITY_ALERT_NOTIFICATION_EMAILS")"
...
if [ -z "$TRUSTED_BFF_EXPECTED_SERVICE_ACCOUNT_EMAIL" ]; then
    TRUSTED_BFF_EXPECTED_SERVICE_ACCOUNT_EMAIL="$FRONTEND_SERVICE_ACCOUNT_EMAIL"
fi
...
if [ -z "$DOCUMENT_INTAKE_KMS_LOCATION" ]; then
    DOCUMENT_INTAKE_KMS_LOCATION="$REGION"
fi
...
if [ -z "$TRUSTED_BFF_IDENTITY_TOKEN_AUDIENCE" ]; then
    TRUSTED_BFF_IDENTITY_TOKEN_AUDIENCE="$CANONICAL_RUN_APP_BASE_URL"
fi
...
validate_cloud_run_execution_environment() {
    local VALUE
    VALUE="$(printf '%s' "$1" | tr '[:upper:]' '[:lower:]')"
    [[ "$VALUE" = "gen1" || "$VALUE" = "gen2" ]]
}
...
ensure_kms_keyring_and_crypto_key() {
    if ! gcloud kms keyrings describe "$DOCUMENT_INTAKE_KMS_KEYRING" \
        --location "$DOCUMENT_INTAKE_KMS_LOCATION" \
        --project "$PROJECT_ID" >/dev/null 2>&1; then
        echo -e "   🔐 Creating KMS key ring: ${YELLOW}$DOCUMENT_INTAKE_KMS_KEYRING${NC}"
        gcloud kms keyrings create "$DOCUMENT_INTAKE_KMS_KEYRING" \
            --location "$DOCUMENT_INTAKE_KMS_LOCATION" \
            --project "$PROJECT_ID"
    else
        echo -e "   ✅ KMS key ring exists: ${YELLOW}$DOCUMENT_INTAKE_KMS_KEYRING${NC}"
    fi

    if ! gcloud kms keys describe "$DOCUMENT_INTAKE_KMS_KEY" \
        --keyring "$DOCUMENT_INTAKE_KMS_KEYRING" \
        --location "$DOCUMENT_INTAKE_KMS_LOCATION" \
        --project "$PROJECT_ID" >/dev/null 2>&1; then
        echo -e "   🔐 Creating KMS crypto key: ${YELLOW}$DOCUMENT_INTAKE_KMS_KEY${NC}"
        gcloud kms keys create "$DOCUMENT_INTAKE_KMS_KEY" \
            --keyring "$DOCUMENT_INTAKE_KMS_KEYRING" \
            --location "$DOCUMENT_INTAKE_KMS_LOCATION" \
            --purpose encryption \
            --project "$PROJECT_ID"
    else
        echo -e "   ✅ KMS crypto key exists: ${YELLOW}$DOCUMENT_INTAKE_KMS_KEY${NC}"
    fi

    DOCUMENT_INTAKE_KMS_KEY_NAME="projects/${PROJECT_ID}/locations/${DOCUMENT_INTAKE_KMS_LOCATION}/keyRings/${DOCUMENT_INTAKE_KMS_KEYRING}/cryptoKeys/${DOCUMENT_INTAKE_KMS_KEY}"
}

grant_kms_role_on_document_intake_key() {
    local MEMBER_EMAIL=$1
    local ROLE_NAME=$2

    echo -e "   🔐 Granting ${YELLOW}${ROLE_NAME}${NC} on document intake key to ${YELLOW}${MEMBER_EMAIL}${NC}"
    gcloud kms keys add-iam-policy-binding "$DOCUMENT_INTAKE_KMS_KEY" \
        --keyring "$DOCUMENT_INTAKE_KMS_KEYRING" \
        --location "$DOCUMENT_INTAKE_KMS_LOCATION" \
        --project "$PROJECT_ID" \
...
deploy_cleanup_service() {
    ...
    gcloud run deploy "$CLEANUP_SERVICE_NAME" \
        ...
        --set-env-vars "GCS_BUCKET_NAME=${BUCKET_NAME},FIRESTORE_DATABASE_ID=${FIRESTORE_DATABASE_ID},FIRESTORE_COLLECTION_NAME=${FIRESTORE_COLLECTION_NAME},FIRESTORE_FILES_SUBCOLLECTION=${FIRESTORE_FILES_SUBCOLLECTION},TMPDIR=${CLEANUP_SERVICE_TMPDIR}"
}
...
deploy_document_av_scanner_service() {
    ...
    gcloud run deploy "$DOCUMENT_AV_SCANNER_SERVICE_NAME" \
        ...
        --set-env-vars "^__ENV_DELIM__^NODE_ENV=${NODE_ENV}__ENV_DELIM__DOCUMENT_AV_SCANNER_MAX_BYTES=${DOCUMENT_AV_SCANNER_MAX_BYTES}__ENV_DELIM__CLAMAV_HOST=${CLAMAV_HOST}__ENV_DELIM__CLAMAV_PORT=${CLAMAV_PORT}__ENV_DELIM__CLAMAV_TIMEOUT_MS=${CLAMAV_TIMEOUT_MS}__ENV_DELIM__TMPDIR=${DOCUMENT_AV_SCANNER_TMPDIR}"
}
...
ensure_app_service_account_and_permissions() {
    ...
    grant_kms_role_on_document_intake_key "$APP_SERVICE_ACCOUNT_EMAIL" "roles/cloudkms.cryptoKeyEncrypter"
}

ensure_document_job_worker_service_account_and_permissions() {
    if ! gcloud iam service-accounts describe "$DOCUMENT_JOB_WORKER_SERVICE_ACCOUNT_EMAIL" --project "$PROJECT_ID" >/dev/null 2>&1; then
        echo -e "   🔐 Creating dedicated document worker service account: ${YELLOW}$DOCUMENT_JOB_WORKER_SERVICE_ACCOUNT_NAME${NC}"
        gcloud iam service-accounts create "$DOCUMENT_JOB_WORKER_SERVICE_ACCOUNT_NAME" \
            --project "$PROJECT_ID" \
            --display-name "Document Job Worker Service"
    else
        echo -e "   ✅ Document worker service account exists: ${YELLOW}$DOCUMENT_JOB_WORKER_SERVICE_ACCOUNT_EMAIL${NC}"
    fi
    ...
    while [ "$ATTEMPT" -le "$MAX_ATTEMPTS" ]; do
        ...
        echo -e "   ⏳ Waiting for document worker service account propagation (${ATTEMPT}/${MAX_ATTEMPTS})..."
        sleep 3
        ATTEMPT=$((ATTEMPT + 1))
    done

    if ! gcloud iam service-accounts describe "$DOCUMENT_JOB_WORKER_SERVICE_ACCOUNT_EMAIL" --project "$PROJECT_ID" >/dev/null 2>&1; then
        echo -e "${RED}❌ Document worker service account did not become available in time: $DOCUMENT_JOB_WORKER_SERVICE_ACCOUNT_EMAIL${NC}"
        exit 1
    fi

    echo -e "   🔐 Granting bucket-scoped storage.objectUser to document worker service account"
    gcloud storage buckets add-iam-policy-binding "gs://$BUCKET_NAME" \
        --member="serviceAccount:$DOCUMENT_JOB_WORKER_SERVICE_ACCOUNT_EMAIL" \
        --role="roles/storage.objectUser" \
        --quiet >/dev/null
    remove_bucket_role_binding_if_exists "gs://$BUCKET_NAME" "$DOCUMENT_JOB_WORKER_SERVICE_ACCOUNT_EMAIL" "roles/storage.objectAdmin"

    add_project_iam_policy_binding_if_enabled \
        "serviceAccount:$DOCUMENT_JOB_WORKER_SERVICE_ACCOUNT_EMAIL" \
        "roles/datastore.user" \
        "datastore.user to document worker service account"
...
ensure_document_job_worker_and_scheduler() {
    ...
    echo -e "   Worker service account  : ${YELLOW}$DOCUMENT_JOB_WORKER_SERVICE_ACCOUNT_EMAIL${NC}"
...
prompt_value_if_empty() {
    ...
    local ALLOW_DEFAULT_IN_NON_INTERACTIVE=${5:-false}
    ...
    if [ "$DEPLOY_NON_INTERACTIVE" = "true" ]; then
        if [ "$ALLOW_DEFAULT_IN_NON_INTERACTIVE" = "true" ] && [ -n "$DEFAULT_VALUE" ]; then
            printf -v "$VAR_NAME" '%s' "$DEFAULT_VALUE"
            return
        fi

        echo -e "${RED}❌ Missing required config for non-interactive deploy: $VAR_NAME${NC}"
        exit 1
    fi
...
prompt_for_missing_config() {
    ...
    prompt_value_if_empty SMTP_PORT "SMTP port" "465" "false" "true"
    prompt_value_if_empty SMTP_SECURE "SMTP secure (true/false)" "true" "false" "true"
    ...
    prompt_value_if_empty SMTP_FROM_NAME_VALUE "SMTP from name" "AI FormCheck Support" "false" "true"
    reuse_oidc_credentials_from_secret_manager
...
ensure_smtp_password() {
    ...
    if [ "$DEPLOY_NON_INTERACTIVE" = "true" ]; then
        echo -e "${RED}❌ SMTP_PASS_VALUE is required for non-interactive deploy when Secret Manager has no existing value.${NC}"
        exit 1
    fi
...
validate_smtp_config() {
    ...
    if [ -z "$DOCUMENT_INTAKE_KMS_LOCATION" ] || [ -z "$DOCUMENT_INTAKE_KMS_KEYRING" ] || [ -z "$DOCUMENT_INTAKE_KMS_KEY" ]; then
        echo -e "${RED}❌ Document intake KMS config is incomplete.${NC}"
        exit 1
    fi
    ...
    if ! validate_boolean_string "$PFS_V2_ENABLED"; then
        echo -e "${RED}❌ Invalid PFS_V2_ENABLED: $PFS_V2_ENABLED (must be true or false)${NC}"
        exit 1
    fi

    if ! validate_positive_integer "$PFS_V2_HANDSHAKE_TTL_MS"; then
        echo -e "${RED}❌ Invalid PFS_V2_HANDSHAKE_TTL_MS: $PFS_V2_HANDSHAKE_TTL_MS (must be a positive integer)${NC}"
        exit 1
    fi
    ...
    if ! validate_boolean_string "$TRUSTED_BFF_REQUIRE_IDENTITY_TOKEN"; then
        echo -e "${RED}❌ Invalid TRUSTED_BFF_REQUIRE_IDENTITY_TOKEN: $TRUSTED_BFF_REQUIRE_IDENTITY_TOKEN (must be true or false)${NC}"
        exit 1
    fi
    ...
    if [ -z "$TRUSTED_BFF_IDENTITY_TOKEN_HEADER" ]; then
        echo -e "${RED}❌ TRUSTED_BFF_IDENTITY_TOKEN_HEADER is required.${NC}"
        exit 1
    fi
    ...
    if ! validate_cloud_run_execution_environment "$CLOUD_RUN_EXECUTION_ENVIRONMENT"; then
        echo -e "${RED}❌ Invalid CLOUD_RUN_EXECUTION_ENVIRONMENT: $CLOUD_RUN_EXECUTION_ENVIRONMENT (must be gen1 or gen2)${NC}"
        exit 1
    fi
    ...
    if ! validate_boolean_string "$DEPLOY_NON_INTERACTIVE"; then
        echo -e "${RED}❌ Invalid DEPLOY_NON_INTERACTIVE: $DEPLOY_NON_INTERACTIVE (must be true or false)${NC}"
        exit 1
    fi
    ...
    if ! validate_boolean_string "$ENABLE_SECURITY_LOG_METRICS"; then
        echo -e "${RED}❌ Invalid ENABLE_SECURITY_LOG_METRICS: $ENABLE_SECURITY_LOG_METRICS (must be true or false)${NC}"
        exit 1
    fi

    if ! validate_boolean_string "$ENABLE_SECURITY_ALERT_POLICIES"; then
        echo -e "${RED}❌ Invalid ENABLE_SECURITY_ALERT_POLICIES: $ENABLE_SECURITY_ALERT_POLICIES (must be true or false)${NC}"
        exit 1
    fi

    if ! validate_boolean_string "$AUTO_CREATE_SECURITY_NOTIFICATION_CHANNELS"; then
        echo -e "${RED}❌ Invalid AUTO_CREATE_SECURITY_NOTIFICATION_CHANNELS: $AUTO_CREATE_SECURITY_NOTIFICATION_CHANNELS (must be true or false)${NC}"
        exit 1
    fi
    ...
    if ! [[ "$AI_CHAT_DAILY_TOKEN_LIMIT" =~ ^[0-9]+$ ]] || [ "$AI_CHAT_DAILY_TOKEN_LIMIT" -le 0 ]; then
        echo -e "${RED}❌ Invalid AI_CHAT_DAILY_TOKEN_LIMIT: $AI_CHAT_DAILY_TOKEN_LIMIT${NC}"
        exit 1
    fi

    if ! [[ "$AI_VALIDATION_DAILY_TOKEN_LIMIT" =~ ^[0-9]+$ ]] || [ "$AI_VALIDATION_DAILY_TOKEN_LIMIT" -le 0 ]; then
        echo -e "${RED}❌ Invalid AI_VALIDATION_DAILY_TOKEN_LIMIT: $AI_VALIDATION_DAILY_TOKEN_LIMIT${NC}"
        exit 1
    fi
    ...
    if ! validate_tmpdir_path "$BACKEND_TMPDIR"; then
        echo -e "${RED}❌ Invalid BACKEND_TMPDIR: $BACKEND_TMPDIR (must be an absolute path)${NC}"
        exit 1
    fi

    if ! validate_tmpdir_path "$DOCUMENT_JOB_WORKER_TMPDIR"; then
        echo -e "${RED}❌ Invalid DOCUMENT_JOB_WORKER_TMPDIR: $DOCUMENT_JOB_WORKER_TMPDIR (must be an absolute path)${NC}"
        exit 1
    fi

    if ! validate_tmpdir_path "$DOCUMENT_AV_SCANNER_TMPDIR"; then
        echo -e "${RED}❌ Invalid DOCUMENT_AV_SCANNER_TMPDIR: $DOCUMENT_AV_SCANNER_TMPDIR (must be an absolute path)${NC}"
        exit 1
    fi

    if ! validate_tmpdir_path "$CLEANUP_SERVICE_TMPDIR"; then
        echo -e "${RED}❌ Invalid CLEANUP_SERVICE_TMPDIR: $CLEANUP_SERVICE_TMPDIR (must be an absolute path)${NC}"
        exit 1
    fi
    ...
    if [ "$ENABLE_DOCUMENT_JOB_WORKER" = "true" ] && [ "$DOCUMENT_JOB_PROCESSING_TIMEOUT_MS" -ge $((DOCUMENT_JOB_WORKER_TIMEOUT_SECONDS * 1000)) ]; then
        echo -e "${RED}❌ DOCUMENT_JOB_PROCESSING_TIMEOUT_MS must be less than DOCUMENT_JOB_WORKER_TIMEOUT_SECONDS to allow app-level timeout/cleanup before Cloud Run terminates the worker${NC}"
        exit 1
    fi

    if [ "$ENABLE_DOCUMENT_AV_SCANNER" = "true" ] && [ "$DOCUMENT_AV_SCAN_TIMEOUT_MS" -ge $((DOCUMENT_AV_SCANNER_TIMEOUT_SECONDS * 1000)) ]; then
        echo -e "${RED}❌ DOCUMENT_AV_SCAN_TIMEOUT_MS must be less than DOCUMENT_AV_SCANNER_TIMEOUT_SECONDS to allow app-level timeout handling before Cloud Run terminates the AV scanner${NC}"
        exit 1
    fi
}
...
print_deploy_summary() {
    ...
    echo -e "   Execution Environment   : ${YELLOW}$CLOUD_RUN_EXECUTION_ENVIRONMENT${NC}"
    ...
    echo -e "   Backend TMPDIR          : ${YELLOW}$BACKEND_TMPDIR${NC}"
    echo -e "   Worker TMPDIR           : ${YELLOW}$DOCUMENT_JOB_WORKER_TMPDIR${NC}"
    echo -e "   AV Scanner TMPDIR       : ${YELLOW}$DOCUMENT_AV_SCANNER_TMPDIR${NC}"
    ...
    echo -e "   AI Chat Token Limit     : ${YELLOW}$AI_CHAT_DAILY_TOKEN_LIMIT${NC}"
    echo -e "   AI Validation Token Lim.: ${YELLOW}$AI_VALIDATION_DAILY_TOKEN_LIMIT${NC}"
    ...
    echo -e "   PFS V2 Enabled          : ${YELLOW}$PFS_V2_ENABLED${NC}"
    echo -e "   PFS V2 TTL (ms)         : ${YELLOW}$PFS_V2_HANDSHAKE_TTL_MS${NC}"
    ...
    echo -e "   Trusted BFF ID Token    : ${YELLOW}$TRUSTED_BFF_REQUIRE_IDENTITY_TOKEN${NC}"
    echo -e "   Trusted BFF ID Header   : ${YELLOW}$TRUSTED_BFF_IDENTITY_TOKEN_HEADER${NC}"
    echo -e "   Trusted BFF SA Expect   : ${YELLOW}${TRUSTED_BFF_EXPECTED_SERVICE_ACCOUNT_EMAIL:-<unset>}${NC}"
    echo -e "   Trusted BFF Audience    : ${YELLOW}${TRUSTED_BFF_IDENTITY_TOKEN_AUDIENCE:-<unset>}${NC}"
    ...
    echo -e "   Deploy Non-Interactive  : ${YELLOW}$DEPLOY_NON_INTERACTIVE${NC}"
    ...
    echo -e "   Security Log Metrics    : ${YELLOW}$ENABLE_SECURITY_LOG_METRICS${NC}"
    echo -e "   Security Alert Policies : ${YELLOW}$ENABLE_SECURITY_ALERT_POLICIES${NC}"
    echo -e "   Alert Policy Prefix     : ${YELLOW}$SECURITY_ALERT_POLICY_PREFIX${NC}"
    echo -e "   Auto Create Alert Ch    : ${YELLOW}$AUTO_CREATE_SECURITY_NOTIFICATION_CHANNELS${NC}"
    if [ -n "$SECURITY_ALERT_NOTIFICATION_EMAILS" ]; then
        echo -e "   Alert Notification Mail : ${YELLOW}$SECURITY_ALERT_NOTIFICATION_EMAILS${NC}"
    fi
...
run_preflight_checks() {
    ...
    if [ "$TRUSTED_BFF_REQUIRE_IDENTITY_TOKEN" = "true" ] && { [ -z "$TRUSTED_BFF_EXPECTED_SERVICE_ACCOUNT_EMAIL" ] || [ -z "$TRUSTED_BFF_IDENTITY_TOKEN_AUDIENCE" ]; }; then
        echo -e "${RED}   ❌ Trusted BFF identity token mode requires expected service account email and audience.${NC}"
        PRECHECK_FAILED="true"
    fi

    if [ "$TRUSTED_BFF_AUTH_ENABLED" = "true" ] && [ "$TRUSTED_BFF_REQUIRE_IDENTITY_TOKEN" != "true" ]; then
        echo -e "${YELLOW}   ⚠️  Trusted BFF is still using shared-secret-only attestation. Enable TRUSTED_BFF_REQUIRE_IDENTITY_TOKEN=true to tighten service identity verification.${NC}"
    fi
...
run_security_observability_setup() {
    if [ "$ENABLE_SECURITY_LOG_METRICS" != "true" ] && [ "$ENABLE_SECURITY_ALERT_POLICIES" != "true" ]; then
        echo -e "   ℹ️  Skipping security observability setup because both ENABLE_SECURITY_LOG_METRICS and ENABLE_SECURITY_ALERT_POLICIES are disabled"
        return
    fi

    if [ ! -x "$SCRIPT_DIR/scripts/createSecurityLogMetrics.sh" ]; then
        echo -e "${YELLOW}   ⚠️  Security log metrics script is missing or not executable: $SCRIPT_DIR/scripts/createSecurityLogMetrics.sh${NC}"
        return
    fi

    if [ "$ENABLE_SECURITY_LOG_METRICS" = "true" ]; then
        echo -e "${YELLOW}📈 Ensuring security log metrics...${NC}"
        PROJECT_ID="$PROJECT_ID" \
        BACKEND_SERVICE_NAME="$SERVICE_NAME" \
        "$SCRIPT_DIR/scripts/createSecurityLogMetrics.sh"
    else
        echo -e "   ℹ️  Skipping security log metrics because ENABLE_SECURITY_LOG_METRICS=false"
    fi

    if [ "$ENABLE_SECURITY_ALERT_POLICIES" != "true" ]; then
        echo -e "   ℹ️  Skipping security alert policies because ENABLE_SECURITY_ALERT_POLICIES=false"
        return
    fi

    if [ ! -x "$SCRIPT_DIR/scripts/createSecurityAlertPolicies.sh" ]; then
        echo -e "${YELLOW}   ⚠️  Security alert policy script is missing or not executable: $SCRIPT_DIR/scripts/createSecurityAlertPolicies.sh${NC}"
        return
    fi

    if [ -z "$SECURITY_ALERT_NOTIFICATION_CHANNELS" ] && [ "$AUTO_CREATE_SECURITY_NOTIFICATION_CHANNELS" != "true" ]; then
        echo -e "${YELLOW}   ⚠️  SECURITY_ALERT_NOTIFICATION_CHANNELS is empty. Alert policies will still be created, but without notification channels.${NC}"
    fi

    echo -e "${YELLOW}🚨 Ensuring security alert policies...${NC}"
    PROJECT_ID="$PROJECT_ID" \
    POLICY_PREFIX="$SECURITY_ALERT_POLICY_PREFIX" \
    NOTIFICATION_CHANNELS="$SECURITY_ALERT_NOTIFICATION_CHANNELS" \
    NOTIFICATION_EMAILS="$SECURITY_ALERT_NOTIFICATION_EMAILS" \
    AUTO_CREATE_NOTIFICATION_CHANNELS="$AUTO_CREATE_SECURITY_NOTIFICATION_CHANNELS" \
    "$SCRIPT_DIR/scripts/createSecurityAlertPolicies.sh"
}

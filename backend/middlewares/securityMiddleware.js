...
function isPfsV2EncryptedRequest(req) {
    return req.body?.protocol_version === 'v2'
        && typeof req.body?.server_key_id === 'string'
        && typeof req.body?.client_ephemeral_public_key === 'string'
        && typeof req.body?.iv === 'string'
        && typeof req.body?.tag === 'string'
        && typeof req.body?.payload === 'string';
}

function buildPfsV2RequestContext(req) {
    return {
        method: req.method,
        path: req.path
    };
...
module.exports = async (req, res, next) => { // ✅ เปลี่ยนเป็น async function
    ...
    if (ENCRYPTED_METHODS.includes(req.method)) {
        ...
        if (req.body && ((req.body.encKey && req.body.payload) || isPfsV2EncryptedRequest(req))) {
            const isPfsV2Request = isPfsV2EncryptedRequest(req);
            const result = isPfsV2Request
                ? decryptPfsV2Payload(req.body, buildPfsV2RequestContext(req))
                : decryptHybridPayload(req.body);
            ...
            req.body = data;
            res.locals.sessionKey = result.responseKey || result.aesKey;
            res.locals.encryptionProtocol = isPfsV2Request ? 'v2' : 'v1';
            res.locals.serverKeyId = result.serverKeyId || null;
    ...
    res.json = function (data) {
        if (res.locals.sessionKey) {
            try {
                const encryptedResponse = res.locals.encryptionProtocol === 'v2'
                    ? encryptPfsV2Response(
                        buildEncryptedResponseData(data),
                        res.locals.sessionKey,
                        res.locals.serverKeyId
                    )
                    : encryptSymmetric(
                        buildEncryptedResponseData(data),
                        res.locals.sessionKey
